...
window.onload = function() {
    ...
    // ゲームに使う素材を、あらかじめ読み込む
    game_.preload('./img/chara1.png', './img/chara3.png', './img/map1.png', './img/start.png', './img/gameover.png');
    game_.onload = function() { // ゲームの準備が整ったらメインの処理を実行します
        ...
        */
        var createGameScene = function() {
            ...
            var stageWidth = 4;
            var stageHeight = 3;
            var stagePanelWidth = 16;
            var stagePanelHeight = 16;

            for(indexY = 0; indexY < stageHeight; indexY++){//stage
                for(indexX = 0; indexX < stageWidth; indexX++){
                    //Spriteを作成する
                    index = indexX + indexY * stageHeight;

                    droid[ index ] = new Sprite( stagePanelWidth, stagePanelHeight ) ;
                    droid[ index ].image = game_.assets[ './img/map1.png' ] ;
                    droid[ index ].frame = 12*16+5;//24;//横16画像
                    droid[index].x = 50 + indexX * stagePanelWidth;
                    droid[index].y = 50 + indexY * stagePanelHeight;
